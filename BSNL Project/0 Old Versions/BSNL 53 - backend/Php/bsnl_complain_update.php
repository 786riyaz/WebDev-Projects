...
if ($conn->connect_error) {
    ...
    $stmt = $conn->prepare("UPDATE complain SET Is_Resolved = 1,  complain_solve_remark = ?, complain_solve_date = ?, Resolution_Duration = ?  WHERE Complain_ID = ?");

    // Bind parameters
    $stmt->bind_param("sssi",  $ResolutionRemark, $ResolveDateTime, $Duration, $ID);
